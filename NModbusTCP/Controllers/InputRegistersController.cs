...
namespace ModbusTCP.Controllers
{
    ...
    [ApiController]
    [Authorize]
    public class InputRegistersController : ControllerBase
    {
        ...
        public InputRegistersController(IConfiguration configuration, ILogger<InputRegistersController> logger)
        {
            _logger = logger;
        ...
        public async Task<IActionResult> ReadInputRegistersAsync(ushort offset, ushort number, string address = null, int? port = null, byte? slave = null)
        {
            ModbusRequestData request = new ModbusRequestData(new ModbusSlaveData(_settings, address, port, slave), offset, number);
            return await this.ModbusReadRequest(_logger, request, "ReadInputRegistersAsync");
        }
